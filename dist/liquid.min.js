...
output=String(context.registers['decrement'][key]);context.registers['decrement'][key]--;return output;}}));(function(Liquid){var Money={getCurrencySign:function(){var currencySign='$';return currencySign;},getCurrencyCode:function(){var currencyCode='';return currencyCode?' '+currencyCode:'';},format:function(num,currencySign,trimZero){var ret='0.00';try{num=parseFloat((''+num)||ret);if(trimZero!==true){num=Math.round(num*100)/100;}
var tokens=(''+num).match(/^(-?)(\d+)(.(\d+))?$/)||[];var sign=tokens[1]||'';var digit=tokens[2]||'0';var dp=((tokens[4]||'')+'00');if(trimZero){dp=dp.replace(/0+$/,'');}else{dp=dp.substr(0,2);}
ret=sign+currencySign+digit+(dp.length>0?'.'+dp:'');}catch(e){}
